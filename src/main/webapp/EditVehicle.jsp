  ...
  <div class="row">
    <div class="col-md-12">
      <div class="card card-primary">
        ...
        <form action="updateVehicle.do" class="frmBx" method="post"  autocomplete="off" enctype="multipart/form-data">
<!--         <input type="hidden" name="_token" value="{{ csrf_token() }}">
 -->          <div class="card-body">
          <div class="row">
			<input type="hidden" id="vehicleId" class="form-control" name="vehicleId" placeholder="vehicle Id" value="${VehicleDetails.getVehicleId()}">
<%-- 			<input type="hidden" id="vehicle_partner" class="form-control" name="vehicle_partner" placeholder="vehicle_partner" value="${VehicleDetails.getVehicle_partner()}">
 --%>             <%-- <input type="hidden" id="campaign" class="form-control" name="campaign" placeholder="campaign" value="${VehicleDetails.getCampaign()}">
               --%>
              <input type="hidden" id="status" class="form-control" name="status" placeholder="status" value="${VehicleDetails.getStatus()}">
              
            <div class="col-12 col-sm-4">
              <div class="form-group">
                <label>Vehicle Partner</label>
                <div class="select2-purple">
                   <input type="text" class="form-control" name="owner_name" id="owner_name" value="${VehicleDetails.getOwner_name()}">
                  </div>
              </div>
            </div>
            <div class="col-12 col-sm-4">
              <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="vehicle_number" id="vehicle_number" value="${VehicleDetails.getVehicle_number()}">
              </div>
            </div>
           <div class="col-12 col-md-4">
		                  ...
		                  <select class="form-control select2" name="start_location" style="width: 100%;" placeholder="${VehicleDetails.getStart_location()}">
		                    <!-- <option selected="selected" disabled="disabled">Start Location</option> -->
                        <!-- @foreach($admin_roles['roles'] as $role)
		                    <option value="{{$role['id']}}">{{$role['role']}}</option>
                        @endforeach -->
                       <%--  <option>${VehicleDetails.getStart_location()}</option> --%>
                       <option value="${VehicleDetails.getStart_location()}"  selected>${VehicleDetails.getStart_location()}</option>
             ...
             <%-- <div class="col-12 col-md-4">
              		...
              		jQuery.noConflict();
              			$(".ui.fluid.dropdown").dropdown({
              				...
              				$('.ui.fluid.dropdown').dropdown({'set selected': 'Dadar,Bandra,Worli'});
					  </script>
					  <label>Delivery location</label>
					  <select
						name="delivery_location" multiple="" class="ui fluid dropdown">
						<!-- <option value="">Location</option> -->
						<option value="${VehicleDetails.getDelivery_location()}" selected>${VehicleDetails.getDelivery_location()}</option>
              		 ...
              		 --%>	
              		 <c:forEach items="${zoneList}" var="allZone"> 
                  			<c:choose>
                  			<c:when test="${allZone.getZone_name() != AreaDetails.getZone_name()}">
                  			<option>
                  			
                  			${allZone.getZone_name()}</option>
                  			</c:when>
                  			</c:choose>
                  			</c:forEach>
              		 <div class="col-12 col-sm-4">
             ...
             <div class="col-12 col-sm-4">
              <div class="form-group">
                  ...
                  <script type="text/javascript">
					...
					<input list="browsers" name="platform" id="platform" class="form-control" value="${VehicleDetails.getPlatform()}"><%-- placeholder="${VehicleDetails.getPlatform()}" --%>
						<datalist id="browsers">
						<%-- <option  value="${VehicleDetails.getPlatform()}" selected>${VehicleDetails.getPlatform()}</option> --%>
						  <option value="Amazon">
...
<!--                   <input type="text" name="myText" value="Norway" selectBoxOptions="Canada;Denmark;Finland;Germany;Mexico;Norway;Sweden;United Kingdom;United States">
 -->                  <!-- <input type="text" name="platform" selectBoxOptions="Amazon;Flipkart;Myntra;"> -->
            ...
            <div class="col-12 col-sm-4">
              ...
              <%-- <label>RC Book</label>
                <div class="select2-purple">
                   <input type="file" class="custom-file-input" name="file_type" id="file_type" value="${FileDetails.getFileName()}">
                ...
                <div class="input-group">
                  <div class="custom-file">
                    ...
                    <label class="custom-file-label" for="exampleInputFile">${FileDetails.getFileName()}</label>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-12 col-sm-4">
              ...
              <br>
                            <label class="checkbox-inline"><input id="shift" name="shift" type="checkbox" value="${VehicleDetails.getShift()}" checked="checked">${VehicleDetails.getShift()}</label>&nbsp;&nbsp;&nbsp;
            ...
            <%--   <c:choose>
                  			<c:when test="${VehicleDetails.getShift() == 'Morning'}">
                  				<label class="checkbox-inline"><input id="shift2" name="shift" type="checkbox" value="Afternoon"> Afternoon</label>&nbsp;&nbsp;&nbsp;
             					<label class="checkbox-inline"><input id="shift3" name="shift" type="checkbox" value="Evening"> Evening</label>
                  			</c:when>
                  			</c:choose> --%>
